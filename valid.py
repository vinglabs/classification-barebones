...
def calculate_class_wise_precision_recall_f1(test_predictions,test_labels,classes):
    ...
    for i,class_name in enumerate(classes):
        ...
        stat_dict[class_name]['precision'] = stat_dict[class_name]['tp']/(stat_dict[class_name]['tp'] + stat_dict[class_name]['fp']+0.0001)
        stat_dict[class_name]['recall'] = stat_dict[class_name]['tp']/(stat_dict[class_name]['tp'] + stat_dict[class_name]['fn']+ 0.0001)
        stat_dict[class_name]['f1'] = 2 * stat_dict[class_name]['precision'] * stat_dict[class_name]['recall']/(stat_dict[class_name]['precision'] + stat_dict[class_name]['recall']+0.0001)

    stat_dict['accuracy'] = test_labels.eq(test_predictions).nonzero().size(0)/test_labels.size(0)
